...
def segmentStableNotesRegions(inputFile = '../../sounds/sax-phrase-short.wav', stdThsld=10, minNoteDur=0.1, 
    ...
    for i in range(winStable, len(f0_cents)):
        std.append(np.std(f0_cents[i-winStable:i]))

    #3. apply threshold on standard deviation values to find indexes of the stable points in melody
    # std_i = [i for i in range(len(std)) if std[i] < stdThsld]
    std_i = []
    for i in range(winStable-1,len(std)):
        if std[i] < stdThsld:
            std_i.append(i)
    ...
    for i in std_i:
        if prev_i + 1 != i:
            if len(sub_array) > 1:
                segments = np.append(segments,sub_array[0])
                segments = np.append(segments,sub_array[-1])
                sub_array = []
            else:
                sub_array = []
        sub_array.append(i)
        prev_i = i
    segments_reshaped = segments.reshape((-1, 2))

    #5. apply segment filtering, i.e. remove segments with are < minNoteDur in length
    min_samples = minNoteDur * fs / H
    segments_filtered = np.array([])
    for row in range(segments_reshaped.shape[0]):
        if abs(segments_reshaped[row,1] - segments_reshaped[row, 0]) > min_samples:
            segments_filtered = np.reshape(np.append(segments_filtered, segments_reshaped[row]), (-1, 2))

    # plotSpectogramF0Segments(x, fs, w, N, H, f0, segments)  # Plot spectrogram and F0 if needed
    # plotSpectogramF0Segments(x, fs, w, N, H, f0, segments_reshaped)
...
# testcase = loadTestCases.load(2,1)
# output = segmentStableNotesRegions(**testcase["input"])
