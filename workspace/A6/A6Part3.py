...
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.1, t2=0.5, window='hamming', 
    ...
    #1. Compute harmonic frequencies and magnitudes

    fH, mH, pH = HM.harmonicModelAnal(x, fs, w, N, H, t, nH, minf0, maxf0, f0et, harmDevSlope=0.01, minSineDur=0.0)

    #2. Extract time segment you need (t1 - t2)
    
    min_sample = math.floor(t1 * fs / float(H))
    max_sample = math.ceil(t2 * fs / float(H))

    freqH = fH[min_sample: max_sample+1, :]
    
    # #3. Compute mean inhamonicity

    # inharm_vec = []

    # for frame in range(freqH.shape[0]):
    #     inharm = 0
    #     for harm in range(1, freqH.shape[1]):
    #         inharm += np.abs(freqH[frame][harm] - (harm + 1) * freqH[frame][0]) / (harm+1)
    #     inharm = inharm / (nH)
    #     inharm_vec = np.append(inharm_vec, inharm)
    
    # inharm_mean = 1/(max_sample - min_sample) * np.sum(inharm_vec, axis = 0)

    #3. Compute mean inharmonicity - Vectorized Version

    theory_freq = freqH[:, 0]
    theory_freq = np.reshape(theory_freq, (freqH.shape[0], 1))* np.arange(1, freqH.shape[1]+1)    # matrix teórica frames x harmónicos s/fundamental
    harm_num = np.arange(1.0, freqH.shape[1])
    harm_num = np.reshape(harm_num, (1, freqH.shape[1]-1))                                          # vector de indices de harmonicos, de 1 a 24

    inharm_matrix = np.abs(freqH[:, 1:] - (theory_freq[:, 1:])) / (harm_num+1)
    inharm_matrix = np.sum(inharm_matrix, axis=1) / float(harm_num.shape[1]+1)
    inharm_mean = 1.0/float(max_sample - min_sample+1) * np.sum(inharm_matrix)

    # input(inharm_mean)                    
    
    return inharm_mean

# testcase = loadTestCases.load(3, 2)
# output = estimateInharmonicity(**testcase["input"])
