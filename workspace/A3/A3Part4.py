...
def suppressFreqDFTmodel(x, fs, N):
    ...
    mX, pX = dftAnal(x, w, N)
    bin70 = int(np.ceil(70 * N / fs))
    mX_filt = mX.copy()
    mX_filt[: bin70 + 1] = -120

    y = dftSynth(mX, pX, w.size) * outputScaleFactor
    yfilt = dftSynth(mX_filt, pX, w.size) * outputScaleFactor

    return y, yfilt


# # example signal x => sum of two sinusoids x1 and x2
# fs = 48000
# f1 = 40
# f2 = 100
# f3 = 200
...
# N = 1024
# n = np.arange(N)
# x1 = np.cos(2 * np.pi * f1 * n * 1 / fs)
# x2 = np.cos(2 * np.pi * f2 * n * 1 / fs)
# x3 = np.cos(2 * np.pi * f3 * n * 1 / fs)
# x4 = np.cos(2 * np.pi * f4 * n * 1 / fs)
# x = x1 + x2 + x3 + x4
# (x, fs, f1, f2)
# plt.plot(x)
# plt.show(block = True)
