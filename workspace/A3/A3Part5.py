...
def zpFFTsizeExpt(x, fs):
    ...
    mX1, pX1 = dftAnal(xseg, w1, N=256)
    mX2, pX2 = dftAnal(x, w2, N=512)
    mX3, pX3 = dftAnal(xseg, w1, N=512)
    ...
    freq_vec1 = np.arange(N1) * fs / (N1*2)
    freq_vec2 = np.arange(N2) * fs / (N2*2)
    freq_vec3 = np.arange(N3) * fs / (N3*2)

    # fig, axs = plt.subplots(3)
    # axs[0].plot(freq_vec1, mX1)
    # axs[1].plot(freq_vec2, mX2)
    # axs[2].plot(freq_vec3, mX3)
    # plt.show()
