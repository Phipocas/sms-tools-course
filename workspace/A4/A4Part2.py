...
def computeSNR(inputFile, window, M, N, H):
        ...
        fs, x = UF.wavread(inputFile)                       # input signal
        w = get_window(window, M, False)
        y = stft.stft(x, w, N, H)                    # output signal
        ...
        # e_in = float(np.dot(np.abs(x), np.transpose(np.abs(x))))
        # e_noise = float(np.dot(np.abs(noise), np.transpose(np.abs(noise))))
        e_in = sum(abs(x ** 2))
        e_noise = sum(abs(noise ** 2))

        SNR1 = 10 * math.log10(e_in / e_noise + eps)
        ...
        SNR2 = 10 * np.log10(e_in / e_noise + eps)

        return (SNR1, SNR2)


# computeSNR("/Users/marcoferreira/Now/Audio DSP/sms-tools-master/sounds/piano.wav", "blackman", 513, 2048, 128)