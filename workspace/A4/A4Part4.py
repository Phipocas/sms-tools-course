...
def computeODF(inputFile, window, M, N, H):
        ...
        low_freq = 3000
        high_freq = 10000
        k_low = math.ceil(low_freq * float(N) / fs)
        k_high = math.ceil(high_freq * float(N) / fs)                  

        # compute energy arrays

        mX= np.power(10, mX / 20)       # MUST CONVERT FROM DB TO MAGNITUDE VALUES BEFORE CONVERTING TO ENERGY
        e_low = np.sum(np.square(mX[:, 1:k_low]), 1)
        e_high = np.sum(np.square(mX[:, k_low:k_high]), 1)

        # convert them to dB
        e_low = 10*np.log10(e_low)
        e_high = 10 * np.log10(e_high)
        e_low = e_low.reshape((1, len(e_low)))
        e_high = e_high.reshape((1, len(e_high)))

        #initialize ODF's

        odf_low = np.zeros((1, e_low.shape[1]))
        odf_high = np.zeros((1, e_high.shape[1]))

        # compute ODF's
        for i in range(e_low.shape[1]-1):
                odf_low[0][i + 1] = e_low[0][i + 1] - e_low[0][i]
                odf_high[0][i + 1] = e_high[0][i + 1] - e_high[0][i]

        # Half wave rectification
        odf_low = np.where(odf_low > 0, odf_low, 0)
        odf_high = np.where(odf_high > 0, odf_high,  0)

        ODF = np.concatenate((odf_low, odf_high)).T

        return ODF

testcase = loadTestCases.load(4, 2)
output = computeODF(**testcase["input"])
print(output, "\n")
