...
"""
def plotSpect(mX, H, N, fs, pX, engEnv):
        ...
        plt.subplot(312)                    
        plt.pcolormesh(np.diff(np.transpose(pX),axis=0))
        plt.autoscale(tight=True)
...
def computeEngEnv(inputFile, window, M, N, H):
        ...
        engEnv = np.transpose(np.array([e_low, e_high]))
        # plotSpect(mX, H, N, fs, pX, engEnv)
        return engEnv

# computeEngEnv("/Users/marcoferreira/Now/Audio DSP/sms-tools-master/sounds/piano.wav", "blackman", 513, 1024, 128)