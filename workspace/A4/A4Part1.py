...
def extractMainLobe(window, M):
        ...
        N = M * 8
        w = np.append(w, np.zeros((1, N-M)))

        dftbuffer = np.zeros(N)

        dftbuffer[: int(N / 2) ] = w[int(N / 2) :]
        dftbuffer[int(N / 2)  :] = w[: int(N / 2)]
        X = fft(w)
        mX = 20 * np.log10(np.abs(X) + 1e-16)
        

        mX_sampled = np.array([])
        for i in range(N):
                if mX[i + 1] > mX[i]:
                        mX_sampled = np.append(mX_sampled, mX[i])
                        break
                else:
                        mX_sampled = np.append(mX_sampled, mX[i])
        mX_reverse = mX_sampled[::-1]
        mX_reverse = mX_reverse[0:-1]
        
        mX_sampled = np.append(mX_reverse, mX_sampled)
        

        return mX_sampled


# extractMainLobe("hamming", 256)